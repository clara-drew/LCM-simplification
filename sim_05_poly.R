...
for(j in 1:length(maxT)){
  ...
  for(it in 1:its){
    ...
    write.csv(df_t1,paste("pt305/df_","t1_",maxT[j],"_fm.csv",sep=""))
    write.csv(df_t2,paste("pt305/df_","t2_",maxT[j],"_fm.csv",sep=""))
    df_t1_cp= data.frame(t1_hat=t1_hat_CP[[j]])
    df_t2_cp= cbind(t2_hat_CP[[j]])
    colnames(df_t2_cp)=c(paste("t2_hat_",1:maxT[j],sep=""))
    write.csv(df_t1_cp,paste("pt305/df_","t1_cp_",maxT[j],"_fm.csv",sep=""))
    write.csv(df_t2_cp,paste("pt305/df_","t2_cp_",maxT[j],"_fm.csv",sep=""))
