...
for(i in 1:4){
  ...
  for(j in 1:6){
    ...
    df_t1_fm = read.csv(paste("t3",theta3[i],"/df_","t1_",maxT[j],"_fm.csv",sep=""))
    ...
    df_t1_fm = read.csv(paste("t3",theta3[i],"/df_","t1_cp_",maxT[j],"_fm.csv",sep=""))
...
ggsave("plots_t1_fm.png",g,width=10,height=10)
