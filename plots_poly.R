...
for(i in 1:4){
  ...
  for(j in 1:6){
    ...
    df_t1 = read.csv(paste("pt3",theta3[i],"/df_","t1_",maxT[j],".csv",sep=""))
    ...
    t1_hat[[j]] = df_t1$t1_hat
    t2_hat[[j]] = subset(df_t2, select=c(paste("t2_hat_",1:maxT[j],sep="")))

    df_t1 = read.csv(paste("pt3",theta3[i],"/df_","t1_cp_",maxT[j],".csv",sep=""))
...
ggsave("plots_poly_t1.png",g,width=10,height=10)
