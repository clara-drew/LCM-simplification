library(ggplot2)
system("unzip pt30.zip")
system("unzip pt3005.zip")
system("unzip pt301.zip")
system("unzip pt305.zip")
...
for(i in 1:4){
  ...
  for(j in 1:6){
    ...
    t2_hat[[j]] = subset(df_t2, select=c(paste("t2_hat_",1:maxT[j],sep="")))
    ...
    t2_hat_CP[[j]] = subset(df_t2, select=c(paste("t2_hat_",1:maxT[j],sep="")))

    Et2 = (t1*t2+(1-t1)*t3[i])/(t1*mean(1-prod(1-t2))+(1-t1)*mean(1-(1-t3[i])^maxT[j]))
    Vart2 = Et2[mid[j]]*(1-Et2[mid[j]])/100
  
    #MSE[j]=mean((t2_til[[j]][,mid[j]]-t2[mid[j]])^2)
    CP[j]=mean(t2_til_CP[[j]][,mid[j]])
    #Bias[j]=mean(t2_til[[j]][,mid[j]])-t2[mid[j]]

    Bias[j]=Et2[mid[j]]-t2[mid[j]]
    MSE[j]=Vart2+Bias[j]^2

    MSE_p[j]=mean((t2_til_p[[j]][,mid[j]]-t2[mid[j]])^2)
    CP_p[j]=mean(t2_til_p_CP[[j]][,mid[j]])
    Bias_p[j]=mean(t2_til_p[[j]][,mid[j]])-t2[mid[j]]
    ...
    Et2 = (t1*t2[mid[j]]*mean(1-prod(1-t2[-mid]))+(1-t1)*t3[i]*(1-(1-t3[i])^(maxT[j]-1)))/Et1
    Vart2 = Et2*(1-Et2)/100

    #MSE_s[j]=mean((t2_til_s[[j]][,mid[j]]-t2[mid[j]])^2)
    CP_s[j]=mean(t2_til_s_CP[[j]][,mid[j]])
    #Bias_s[j]=mean(t2_til_s[[j]][,mid[j]])-t2[mid[j]]

    Bias_s[j]=Et2-t2[mid[j]]
    MSE_s[j]=Vart2+Bias_s[j]^2

    MSE_s_p[j]=mean((t2_til_s_p[[j]][,mid[j]]-t2[mid[j]])^2)
    CP_s_p[j]=mean(t2_til_s_p_CP[[j]][,mid[j]])
    Bias_s_p[j]=mean(t2_til_s_p[[j]][,mid[j]])-t2[mid[j]]

    MSE_h[j]=mean((t2_hat[[j]][,mid[j]]-t2[mid[j]])^2,na.rm=T)
    CP_h[j]=mean(t2_hat_CP[[j]][,mid[j]],na.rm=T)
    Bias_h[j]=mean(t2_hat[[j]][,mid[j]],na.rm=T)-t2[mid[j]]
}
  bias = c(bias,Bias_h)
  ...
  p1=ggplot(df_mse,aes(x=time_points,y=MSE,color=Estimate,linetype=as.factor(bc))) + geom_line(alpha=0.8)+scale_linetype_discrete(name="Bias Corrected",breaks=c(0,1),labels=c("No","Yes"))+xlab("Number of tests")+ylim(0,.06) + theme(legend.text=element_text(size=10))+theme(aspect.ratio=4/3)+theme(legend.text=element_text(size=8),legend.title=element_text(size=10))
  ...
  p3=ggplot(df_bias,aes(x=time_points,y=Bias,color=Estimate,linetype=as.factor(bc))) + geom_line(alpha=0.8)+scale_linetype_discrete(name="Bias Corrected",breaks=c(0,1),labels=c("No","Yes"))+xlab("Number of tests")+ylim(-.25,.25)+theme(legend.text=element_text(size=8),legend.title=element_text(size=10))

  }

  else{
    
  p1=ggplot(df_mse,aes(x=time_points,y=MSE,color=Estimate,linetype=as.factor(bc))) + geom_line(alpha=0.8)+xlab("Number of tests")+ylim(0,.06)+theme(legend.position="none")+theme(aspect.ratio=4/3)
  ...
  p3=ggplot(df_bias,aes(x=time_points,y=Bias,color=Estimate,linetype=as.factor(bc))) + geom_line(alpha=0.8)+xlab("Number of tests")+ylim(-.25,.25)+theme(legend.position="none")+theme(aspect.ratio=4/3)
...
ggsave("plots_poly_t22.png",g,width=10,height=10)
